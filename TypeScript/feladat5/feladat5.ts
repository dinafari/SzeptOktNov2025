...
function szorzat(tomb: number[]): number{
    return tomb.reduce((acc,szam)=>acc*szam,1);
}
...
function betuDarab(szoveg: string): number{
    const regex=/ +/g;//szüneteket eltávolítja , 

    const osszevont=szoveg.replace(regex,'');
...
function negativak(tomb2: number[]): number[]{
    return tomb2.filter((szam)=>szam<0)
}
